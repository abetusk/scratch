...
function _lsp_uop() {
  ...
  switch (_op) {
    ...
    case 'cos' : rval = Math.cos(aval); break;
    case 'sin' : rval = Math.sin(aval); break;
...
function _lsp_bop() {
  ...
  switch (_op) {
    ...
    case '^' : rval = Math.pow(aval,bval); break;
...
var COMMON_ENV = {
  ...
  "cos" : { "type": "p", "n_param": -1, "func": function() { return _lsp_uop(_lsp_symb('cos'), ...arguments); } },
  "sin" : { "type": "p", "n_param": -1, "func": function() { return _lsp_uop(_lsp_symb('sin'), ...arguments); } },


  "d" : { "type": "p", "n_param": -1, "func" : auto_diff },
...
function _eval(ast, _env) {
  ...
  if (_type == 'a') {
    ...
    //
      ...
      //DEBUG
      /*
      console.log("funcdef env, child_env.id:", _child_env.id);
      if (_child_env.id == 'U356') {
        console.log("  .:", _child_env.id, _child_env.gg);
        console.log("  .par^1:", _child_env.par.id,                 _child_env.par.gg);
        console.log("  .par^2:", _child_env.par.par.id,             _child_env.par.par.gg);
        console.log("  .par^3:", _child_env.par.par.par.id,         _child_env.par.par.par.gg);
        console.log("  .par^4:", _child_env.par.par.par.par.id,     _child_env.par.par.par.par.gg);
        console.log("  .par^5:", _child_env.par.par.par.par.par.id, _child_env.par.par.par.par.par.gg);
      }
      */
      //console.log("FFF.env:", JSON.stringify(_child_env));
...
function lsp_print_ast(ast, _env, _indent) {
  ...
  if (ast.type == 'a') {
    console.log( ws.join(""), ast.type , "{", _lu[ ast.type ], "} ( a /", ast.child.length, ")");
    for (let i=0; i<ast.child.length; i++) {
  ...
  if (ast.type == 's') {
    ...
    if (typeof vv === "undefined") {
      ...
      console.log( ws.join(""), ast.type, "{", _lu[ ast.type ], "} : '" + ast.val.toString() + "' (unbound)");
